...
OUTPUT_ROOT_DIR = Path("/Users/ocean/Desktop/BTC-Poly-5M Data-Q1")
...
LOG_FILE = OUTPUT_ROOT_DIR / "fetch_btc_trades_5m_multi_api_log.txt"
...
def is_market_dir_complete(market_dir: Path) -> bool:
    return (
        ...
        and (market_dir / "trades.json").exists()
    )
...
def main():
    logger.info("=" * 60)
    logger.info("Dome API - BTC 5M Trades（多 API Key 并发）")
    logger.info("=" * 60)
    ...
    def process_one_market(market: Dict) -> bool:
        ...
        for attempt in range(MARKET_FETCH_RETRIES):
            try:
                # 单次调用，condition_id 包含 Up/Down 两侧的全部成交
                trades = fetch_trades(cid, q_start, q_end)
                break
            except Exception as e:
                last_error = e
                wait = 3 * (attempt + 1)
        ...
        with _write_lock:
            ...
            with open(market_dir / "trades.json", "w", encoding="utf-8") as f:
        ...
        logger.info(f"{slug}  {len(trades)} 条成交记录")
        return True
...
- `market_<YYYYMMDD_HHMMSS_UTC>/`：每个市场一个文件夹，使用该 5 分钟市场对应的 UTC 时间戳命名
  ...
  - `trades.json`：该市场全部成交记录数组（含 Up/Down 两侧）
